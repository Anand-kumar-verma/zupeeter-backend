...
const { getDummyData } = require("./controller/dummyData");
      ...
      try {
        setTimeout(async () => {
          ...
          if (res?.data?.data?.[0]) {
            ...
            dataSendToBackend(time, obj, manual_result, isAlreadyHit);
          } else {
            const obj = getDummyData(
        ...
        const obj = getDummyData(Math.floor(Math.random() * (4 - 0 + 1)) + 0);
        dataSendToBackend(time, obj, manual_result, isAlreadyHit);
...
async function dataSendToBackend(time, obj, manual_result, isAlreadyHit) {
